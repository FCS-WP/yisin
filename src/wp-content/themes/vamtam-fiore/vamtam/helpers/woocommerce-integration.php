...
if ( vamtam_has_woocommerce() ) {
				...
				if ( vamtam_theme_supports( 'woocommerce-products--hide-price' ) ) {
					...
					if ( $hide_product_price ) {
						$content = sprintf(
							...
							esc_html__( 'Read More', 'vamtam-fiore' )
						);
					}
				}
		...
		if ( ! function_exists( 'vamtam_products_ordering' ) ) {
			function vamtam_products_ordering() {
				...
				$catalog_orderby_options = apply_filters( 'vamtam_products_filter_order_by', array(
					'menu_order' => __( 'Default sorting', 'vamtam-fiore' ),
					'popularity' => __( 'Sort by popularity', 'vamtam-fiore' ),
					'rating'     => __( 'Sort by average rating', 'vamtam-fiore' ),
					'date'       => __( 'Sort by latest', 'vamtam-fiore' ),
					'price'      => __( 'Sort by price: low to high', 'vamtam-fiore' ),
					'price-desc' => __( 'Sort by price: high to low', 'vamtam-fiore' ),
				) );
				...
				<div class="woocommerce-ordering">
					<span class="woocommerce-ordering__button">
						<span class="woocommerce-ordering__button-label">
							<?php
								echo ! empty( $orderby ) ? $order_current : esc_html__( 'Default', 'vamtam-fiore' )
							?>
						</span>
							<?php
	...
	function vamtam_woocommerce_before_cart() {
		echo '<h4>';
		esc_html_e( 'Your Cart', 'vamtam-fiore' );
		echo '</h4>';
	}
	...
	function vamtam_wc_general_hooks() {
		...
		function vamtam_woosw_after_items( $key, $products ) {
			if ( !! $products ) {
				...
				if ( ! empty( WPCleverWoosw::$localization[ 'empty_message' ] ) ) {
					...
					$str = esc_html__( 'There are no products on the wishlist!', 'vamtam-fiore' );
				}
			...
			esc_html_e( 'Look for the heart to save favorites while you shop.', 'vamtam-fiore' );
			echo '</p>';
			echo '<form><button class="vamtam-start-shopping" formaction="' . esc_attr( get_permalink( wc_get_page_id( 'shop' ) ) ) . '">' . esc_html__( 'Start Shopping', 'vamtam-fiore' ) . '</button></form>';
					...
					function vamtam_woocommerce_product_custom_fields() {
						...
						woocommerce_wp_checkbox(
							array(
								...
								'label'       => __( 'New Product', 'vamtam-fiore' ),
								'description' => __( 'Products marked as new will display the "New" badge.', 'vamtam-fiore' ),
							)
						);
						echo '</div>';
					}
					...
					function vamtam_show_product_new_badge() {
						...
						<?php if ( ! empty( $_vamtam_product_is_new ) ) : ?>
							...
							<?php
								...
								echo apply_filters( 'vamtam_wc_new_badge', '<span class="' . esc_attr( $classes ) .'">' . esc_html__( 'New!', 'vamtam-fiore' ) . '</span>', $post, $product ); ?>
		...
		if ( vamtam_extra_features() && \Vamtam_Elementor_Utils::is_wc_mod_active( 'sale_badge_as_percentage' ) ) {
			if ( VamtamElementorBridge::is_elementor_active() ) {
				if ( ! function_exists( 'vamtam_woocommerce_sale_badge_as_percentage' ) ) {
					function vamtam_woocommerce_sale_badge_as_percentage( $html, $post, $product ) {
						if( $product->is_type( 'variable' ) ) {
							...
							foreach( $prices[ 'price' ] as $key => $price ) {
								...
								if( $prices[ 'regular_price' ][ $key ] !== $price ) {
									// Calculate and set the percentage for each variation on sale.
									$percentages[] = round( 100 - ( floatval( $prices[ 'sale_price' ][ $key ] ) / floatval( $prices[ 'regular_price' ][ $key ] ) * 100 ) );
								}
							}
							// Displays max discount value.
							...
							foreach( $children_ids as $child_id ) {
								...
								if ( $sale_price != 0 || ! empty( $sale_price ) ) {
									// Calculate and set the percentage for each child on sale.
						...
						return '<span class="onsale">' . esc_html__( '-', 'vamtam-fiore' ) . $percentage . '</span>';
					}
				}
			}
			add_filter( 'woocommerce_sale_flash', 'vamtam_woocommerce_sale_badge_as_percentage', 10, 3 );
		}
	}
		...
		class Vamtam_WC_Ajax_Add_To_Cart_Handler {
			...
			public static function handle_grouped_products() {
				...
				if ( ! empty( $items ) ) {
					...
					if ( ! $was_added_to_cart && ! $quantity_set ) {
						wc_add_notice( __( 'Please choose the quantity of items you wish to add to your cart.', 'vamtam-fiore' ), 'error' );
			...
			public static function handle_variable_products() {
				...
				if ( empty( $variation_id ) && $product && $product->is_type( 'variable' ) ) {
					...
					wc_add_notice( sprintf( __( 'Please choose product options by visiting <a href="%1$s" title="%2$s">%2$s</a>.', 'vamtam-fiore' ), esc_url( get_permalink( $product_id ) ), esc_html( $product->get_name() ) ), 'error' );
